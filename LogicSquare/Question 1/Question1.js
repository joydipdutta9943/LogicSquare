function findCaliforniaCafes(search) {
    ...
    const names = cafes.filter(cafe => cafe.name.match(search))
    const values = []
    for (let i = 0; i < names.length; i++) {
        for (let j = 0; j < places.length; j++) {
            if (names[i].place_id === places[j].id) {
                const name = names[i].name;
                const { id, ...place } = places[j]
                values.push({ name, ...place })
            }
        }
    }
    console.log(values)
}
findCaliforniaCafes("") // Use this function to search